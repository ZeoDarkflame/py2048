...
try:
    from msvcrt import getch     #import getch to hold the screen
except:
    print("Install Microsoft C runtime first, error in importing msvcrt")
...
warning = "You cannot do that"  #this warning is used later
board_size = 5
cap_number = 2048
try:
...
while(True):
    movemade = True                             #we use this to prevent the press any button to spawn a 2 bug
    ...
    if(move == b'a'):
        ...
        exit()
    elif(move == b'u'):             #implementing undo stack
    ...
    if movemade == True:
        if(main_board.wincheck()):
            ...
            break
        main_board.fillrandom()
        if not any(row.count(0) != 0 for row in main_board.board):  #this makes a game a bit faster
            if(not main_board.ispossible()):
                main_board.print_board()
                print("You Lost")
                print("Press any key to continue...")
